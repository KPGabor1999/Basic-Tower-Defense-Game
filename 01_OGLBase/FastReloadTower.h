...
class FastReloadTower : public Tower
{
public:
	static int towerLevel;
	static float reloadTime;		//seconds
	static int damage;
	const static int range = 4;		//tiles
	const static int buildingCost = 15;
	static int upgradeCost;

public:
	FastReloadTower(Level* level, int lineIndex, int columnIndex, MeshObject* ownMesh);
